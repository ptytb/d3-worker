(function (global, factory) {
	...
	(factory((global.PageBundle = {})));
}(this, (function (exports) { 'use strict';

function createNode(type, doc, data) {
                ...
                case 'nofrag':
                    {
                        ...
                        if (renderBy != 'nofrag') {
                            let svg_chart = select(container).select('svg');
                            if (!svg_chart.empty()) {
                                doc.appendChild(container.getElementsByTagName('svg')[0]);
                            }
                        }
                        let svg = select(doc).select('svg');
                        if (svg.empty()) {
                            svg = select(doc).append('svg');
                            svg.append('g');
                        }
